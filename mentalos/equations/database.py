...
+++ mentalos/equations/database.py (修改后)
"""MentalOS Equation Database - Comprehensive Physics and Math Equations"""
from mentalos.types.core_types import PrimaryOperation, Bucket, Model, Tool, EquationEntry
...
EQUATION_DATABASE = [
    ...
    EquationEntry(
        ...
        operation_hint=PrimaryOperation.ACCUMULATE,
        bucket_hint=Bucket.ACCUMULATION,
        model_hint=Model.WORK_ENERGY,
        tool_hint=Tool.TRIGONOMETRY,
        keywords=["work", "force", "distance", "angle", "energy"]
    ),
    EquationEntry(
        id="kinetic_energy",
        name="Kinetic Energy",
        ...
        operation_hint=PrimaryOperation.SCALE,
        bucket_hint=Bucket.CONSERVATION,
        model_hint=Model.WORK_ENERGY,
        tool_hint=Tool.ALGEBRAIC_MANIPULATION,
    ...
    EquationEntry(
        ...
        operation_hint=PrimaryOperation.SCALE,
        bucket_hint=Bucket.DYNAMICS,
        model_hint=Model.NEWTONIAN_MECHANICS,
        tool_hint=Tool.ALGEBRAIC_MANIPULATION,
        keywords=["force", "mass", "acceleration", "newton"]
    ),
    EquationEntry(
        id="vector_resolution_x",
        name="Vector Resolution (X-component)",
        formula="F_x = F * cos(theta)",
        variables=["F", "theta"],
        description="Resolve a vector into its x-component",
        operation_hint=PrimaryOperation.TRANSFORM,
        bucket_hint=Bucket.TRANSFORMATION,
        model_hint=Model.NEWTONIAN_MECHANICS,
        tool_hint=Tool.TRIGONOMETRY,
        keywords=["vector", "resolution", "x-component", "cosine", "horizontal"]
    ),
    EquationEntry(
        id="vector_resolution_y",
        name="Vector Resolution (Y-component)",
        formula="F_y = F * sin(theta)",
        variables=["F", "theta"],
        description="Resolve a vector into its y-component",
        operation_hint=PrimaryOperation.TRANSFORM,
        bucket_hint=Bucket.TRANSFORMATION,
        model_hint=Model.NEWTONIAN_MECHANICS,
        tool_hint=Tool.TRIGONOMETRY,
        keywords=["vector", "resolution", "y-component", "sine", "vertical"]
    ),
    EquationEntry(
        id="friction_force",
        name="Friction Force",
        formula="f = mu * N",
        variables=["mu", "N"],
        description="Friction force given coefficient and normal force",
        operation_hint=PrimaryOperation.SCALE,
        bucket_hint=Bucket.DYNAMICS,
        model_hint=Model.NEWTONIAN_MECHANICS,
        tool_hint=Tool.ALGEBRAIC_MANIPULATION,
    ...
    EquationEntry(
        id="velocity_avg",
        name="Average Velocity",
        formula="v_avg = delta_x / delta_t",
        variables=["delta_x", "delta_t"],
        description="Average velocity over a time interval",
        operation_hint=PrimaryOperation.ESTIMATE,
        bucket_hint=Bucket.KINEMATICS,
        model_hint=Model.KINEMATICS_1D,
        tool_hint=Tool.ALGEBRAIC_MANIPULATION,
        keywords=["velocity", "average", "displacement", "time"]
    ),
    EquationEntry(
        id="acceleration_avg",
        name="Average Acceleration",
        formula="a_avg = delta_v / delta_t",
        variables=["delta_v", "delta_t"],
        description="Average acceleration over a time interval",
        operation_hint=PrimaryOperation.ESTIMATE,
        bucket_hint=Bucket.KINEMATICS,
        model_hint=Model.KINEMATICS_1D,
        tool_hint=Tool.ALGEBRAIC_MANIPULATION,
        keywords=["acceleration", "average", "velocity", "time"]
    ),
    EquationEntry(
        id="momentum",
        name="Momentum",
        ...
        operation_hint=PrimaryOperation.SCALE,
        bucket_hint=Bucket.CONSERVATION,
        model_hint=Model.CONSERVATION_MOMENTUM,
        tool_hint=Tool.ALGEBRAIC_MANIPULATION,
...
def search_equations(keywords: list[str]) -> list[EquationEntry]:
    """Search equations by keywords using fuzzy matching."""
    matches = []
    for entry in EQUATION_DATABASE:
        score = sum(1 for keyword in keywords if keyword.lower() in entry.keywords or keyword.lower() in entry.name.lower())
        if score > 0:
